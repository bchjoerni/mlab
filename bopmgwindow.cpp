...
void bopmgWindow::mLabSignal( const QString& cmd )
{
    QString cmdLower = cmd.toLower().trimmed();

    if( cmdLower == EMERGENCY_STOP )
    {
        emergencyStop();
    }
    else if( cmdLower == STOP_SIGNAL )
    {
